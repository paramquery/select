...
const pq = (function ($, pq) {
  ...
  fn.val = function () {
    ...
    var val2 = this.data
      ...
      .map(function (item) {
        var val = item[valIndx];
        return val == null ? item[textIndx] : val;
      });
    return this.multiple ? val2 : val2[0];
  };
  fn._create = function () {
    var that = this,
      o = that.options,
      $select = that.element,
      isSelect = (that.isSelect = $select.is('select')),
      bootstrap = o.bootstrap,
      bts = bootstrap.on,
      name = $select.attr('name'),
      multiple = $select[0].hasAttribute('multiple'),
    ...
    if (isSelect) {
      $button = $(
        [
          "<div class='pq-select-button pq-no-capture pq-border-2 ",
      ...
      that.divClass = $select.attr('class') || '';
      $button = $select;
      $button.addClass(
        'pq-select-button pq-no-capture pq-border-2 ui-state-default ui-widget ui-corner-all',
      );
    }

    $button.html(
      "<span class='pq-icon " +
        (bts ? 'caret' : 'ui-icon ui-icon-triangle-1-s') +
        "'></span>" +
        "<div class='pq-select-text'></div>",
    );

    that.$button = $button;
    that.multiple = multiple ? true : false;
    that.selector = '.pq-select-option.ui-state-enable:visible';
    that._setButtonWidth();

    o.data ? (that.data = o.data) : that._extractData();
  ...
  fn.onWindowScroll = function () {
    if (this.options.closeOnWindowScroll && !this._preventClose) this.close();
  };
  fn.focus = function () {
    var that = this,
      $s = that.$search;
  ...
  fn._move = function (next, paging, homeEnd) {
    ...
    if (homeEnd) newIndx = next ? dataLen - 1 : 0;
    else if (paging) newIndx = next ? indx + rpp : indx - rpp;
    else if (next)
  ...
  fn.focusOption = function (indexV) {
    ...
    if (top < topMenu) scrollTop = top;
    else if (bottom > bottomMenu) scrollTop = bottom - menuHt;
  ...
  fn.search = function (val) {
    ...
    for (; i < len; i++) {
      var rowData = data[i],
        text = (rowData[textIndx] + '').toUpperCase(),
  ...
  fn.onChange = function (indxV) {
    ...
    if (multiple) {
      if (maxSelect && selIndx.length >= maxSelect) {
        if (
          that._trigger('maxSelectReach', null, {
            option: that.$options[indx],
          }) !== false
        ) {
          that.close();
  ...
  fn.select = function (indx, add) {
    ...
    if (that.multiple) {
      ...
      if (add) {
        if (indx2 == -1) selIndx.push(indx);
      } else {
        if (indx2 > -1) selIndx.splice(indx2, 1);
      }
      $option.prop('selected', add);
    } else {
      if (add) {
        if (selIndx.length && selIndx[0] != indx) {
          data[selIndx[0]].selected = false;
        }
        selIndx[0] = indx;
        $option.prop('selected', add);
      } else {
  ...
  fn._createPopup = function () {
    var that = this,
      ...
      bts = bootstrap.on,
      multiple = that.multiple,
      fontSize = that.$button.css('font-size'),
      searchHTML = '',
      headerHTML = '';
    ...
    if (o.search) {
      searchHTML = [
        ...
        bts ? bootstrap.searchIcon : '',
        "'></span>",
        "<div class='pq-select-search-div1'>",

        "<input type='text' class='pq-select-search-input' autocomplete='off' style='height:",
        o.rowHt - 2,
        "px;'/>",
        '</div>',
        '</div>',
    ...
    $popupCont.css({
      ...
      'font-size': parseInt(fontSize) == 0 ? 'inherit' : fontSize,
    });
  ...
  fn.onSelectAll = function (evt) {
    ...
    for (; i < data.length; i++) {
      ...
      if (!rowData.disabled && rowData.selected !== checked) {
        ...
        if ($options) $($options[rowData.pq_indx]).prop('selected', checked);
      }
    }
    that._createMenu();
    that.renderView();
  ...
  fn.getViewPortIndx = function () {
    var menu = this.$menu[0],
      ...
      initV = Math.floor(scrollTop / rowHt),
  ...
  fn.renderView = function () {
    var that = this,
      ...
      searchIndx,
      rowHt = o.rowHt,
      style = function (i, css) {
        return (
          "style='position:absolute;top:" +
          i * rowHt +
          'px;width:100%;left:0;height:' +
          rowHt +
          'px;overflow:hidden;' +
          (css || '') +
          "'"
    ...
    for (i = initV; i <= finalV; i++) {
      ...
      if (rowData) {
        ...
        if (rowData.pq_title) {
          li.push(
            '<label ',
            style(i, rowData.style),
            " class='pq-select-optgroup'",
            " id='",
            that.getOptionId(i),
            "'>",
  ...
  fn._highlight = function (indexV) {
    var self = this,
      ...
      $label =
        indexV != null
          ? self.getOption(indexV)
          : self.$menu.find('.pq-select-option.ui-state-enable:visible:first'),
      label = $label[0];
  ...
  fn.setMenuHt = function () {
    var that = this,
      ...
      rowHt = o.rowHt,
      htTop =
        (that.$popup.find('.pq-select-search-div').outerHeight(true) || 0) +
        (that.$selectAll.outerHeight(true) || 0),
  ...
  fn.open = function () {
    ...
    $(document).on('mousedown' + that.eventNamespace, function (evt) {
      ...
      if (
        !$target.closest(that.$popupCont)[0] &&
        !$target.closest(that.$button)[0]
      ) {
        that.close();
      }
    });
  ...
  fn.setText = function () {
      ...
      tmpl = function (indx) {
        if (deselect) {
          return [
            ...
            indx,
            "'>",
            "<span class='pq-item-close ",
            closeIcon,
            "'></span>",

            "<span class='pq-select-item-text'>",
            data[indx][textIndx],
            '</span>',
            '</span>',
          ].join('');
        } else {
          return data[indx][textIndx];
        }
      },
      selectIndx = that.selectIndx,
      text;
    ...
    if (that.multiple) {
      ...
      var indx = selectIndx[0],
        text = indx != null ? data[indx][textIndx] : '';
    ...
    if (!that.multiple) {
      $selectText
        .find('.pq-select-item-text')

        .css({ maxWidth: $button.width() - 35 });
    }
    setTimeout(function () {
      if (that.uuid) that.positionPopup();
    });
  };
  ...
  fn._destroy = function () {
    ...
    if (this.isSelect) {
      ...
      $button.empty();
      $button.off();
      $button.attr('class', this.divClass);
    }
